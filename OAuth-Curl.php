 ...
 */
class U1_OAuth_Curl extends U1_OAuth {
    ...
    public function __construct($consumerName, $consumerKey = 'ubuntuone', $consumerSecret = 'hammertime') {
        if (!function_exists('curl_exec')) 
            throw new U1_Exception('The PHP curl functions not available!', 0);
     ...
     * @return string 
     */
    public function fetch($uri, $arguments = array(), $method = 'GET', $httpHeaders = array(), $contentType = '') {
		...
		if (!empty($errorno))
			throw new U1_Exception('Curl error: ('.$errorno.') '.$error."\n", 0);
		...
		if ($status>=300) {
			...
			switch ($status) {
				...
				case 304 :
					...
					break;
				case 401 :
					throw new U1_Exception('Bad Request.
						There is something wrong with the request data.
						' . $body["error"] . "\n", 401);
				case 403 :
					throw new U1_Exception('Forbidden.
						This could mean a bad OAuth request, or a file or folder already existing at the target location.
						' . $body["error"] . "\n", 403);
				case 404 :
					throw new U1_Exception('Resource at uri: ' . $uri . ' could not be found. ' .
							$body["error"] . "\n", 404);
				case 507 :
					throw new U1_Exception('This dropbox is full. ' .
							$body["error"] . "\n", 509);
			}
			if (!empty($body["error"]))
				throw new U1_Exception('Error: ('.$status.') '.$body["error"]."\n", 0);	
		}
    ...
    public function getRequestToken($callback = null) {
        ...
        $result = $this->fetch(self::URI_REQUEST_TOKEN, $args, 'POST');
        ...
        if ($result['httpStatus'] == "200") {
            ...
            throw new U1_Exception('We were unable to fetch request tokens. This likely means that your consumer key and/or secret are incorrect.', 403);
        }
    }
    ...
    public function getAccessToken($verifier) {
        $result = $this->fetch(self::URI_ACCESS_TOKEN, array("oauth_verifier" => $verifier), 'POST');
